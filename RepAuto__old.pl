	...
	for ( my $row = 1 ; $row <= $Sheet->get_last_row ; $row++ ) {
    	for ( my $col = 1 ; $col <= $Sheet->get_last_col ; $col++ ) {
        	...
        	if ( $$Sheet->Cells( $row, $col )->{Value} =~ /^~~~/ ) {
                ...
                my ($interval_unit, $interval, $last_date);
				 if($auto_ext =~ m/\(\s?(.+),\s?(.+),\s?(.+)\s?\)/){
					 ($interval_unit, $interval, $last_date) = ($1, $2, $3);
				 }
				if( scalar @a == 1){					# time function
            		if ($a[0] eq 'from'){
						$$Sheet->Cells( $row, $col )->{Value} =
              	    			 DateTime::Format::Oracle->format_datetime($dt_from);
					}
                    elsif($a[0] eq 'to'){
						$$Sheet->Cells( $row, $col )->{Value} =
              	    			 DateTime::Format::Oracle->format_datetime($dt_to);
					}
					else{
					$$Sheet->Cells( $row, $col )->{Value} =
               ...
               if( scalar @a == 6 && $a[-1] =~ /^\@/ ) {				   # built-in func
					...
					if ( $a[-1] eq '@billing' ) {
								$$Sheet->Cells( $row, $col )->{Value} = $c3000_h->get_billing('ADAS_VAL_RAW', @a);
						}
